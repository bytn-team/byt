...
*Setup instructions for a gitian build of BYTN using a Debian VM or physical system.*
...
Clone the git repositories for bytn and gitian and then checkout the bytn version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bytn-crypto/bytn.git
cd bytn
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bytn repository
...
To build BYTN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bytn repository.
...
```bash
    ...
    From https://github.com/bytn-crypto/bytn
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bytn-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bytn-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bytn-build.assert
```
...
[bytn/gitian.sigs](https://github.com/bytn-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
