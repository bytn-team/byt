...
- [Development guidelines](#development-guidelines)
    - [General BYTN Core](#general-bytn-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BYTN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BYTs (tBYT)" on the test network, if you
...
BYTN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bytn-project/bytn/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bytn
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bytn --log_level=test_suite
$ valgrind -v --leak-check=full src/bytnd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bytn.coverage/index.html`.
```
...
- BitcoinMiner : Generates BYTs (if wallet is enabled).
...
However, in open source software such as BYTN Core, where everyone uses
...
pay attention to for reviewers of BYTN Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BYTN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BYTN Core so that they can be integrated
...
file descriptors it will cause problems with BYTN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BYTN-Project/BYTN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bytn-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bytn-cli` and the GUI debug console use this table to determine how to
