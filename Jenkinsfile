...
// which are part of the BYTN repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="bytn-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /bytn-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "bytnpay-bytn/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from bytnpay-bytn/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'bytnpay-bytn/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from bytnpay-bytn/develop"
            }
          }

          if (hasCache) {
            sh "cd /bytn-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /bytn-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /bytn-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /bytn-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /bytn-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /bytn-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /bytn-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
