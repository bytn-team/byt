...
# bytn_hash
RUN git clone https://github.com/bytnpay/bytn_hash
RUN cd bytn_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} bytn
RUN useradd -u ${USER_ID} -g bytn -s /bin/bash -m -d /bytn bytn
...
RUN mkdir /bytn-src && \
  ...
  chown $USER_ID:$GROUP_ID /bytn-src && \
