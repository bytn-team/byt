...
// The zBYT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBYT is a hash of the serial
...
{
    //Create an output returning the zBYT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBYTOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBYT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBYT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBYTOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBYT output", __func__);
...
{
    CzBYTTracker* zbytTracker = pwallet->zbytTracker.get();
    CMintMeta meta;
    if (!zbytTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbytTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BYT stake is the outpoint
