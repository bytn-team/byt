...
CzBYTTracker::CzBYTTracker(std::string strWalletFile)
{
...
CzBYTTracker::~CzBYTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBYTTracker::Init()
{
...
bool CzBYTTracker::Archive(CMintMeta& meta)
{
...
bool CzBYTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBYTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBYTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBYTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBYTTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBYTTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBYTTracker::GetSerialHashes()
{
...
CAmount CzBYTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBYTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBYTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBYTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBYTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBYTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBYTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBYTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBYTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBYTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBYTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBYTWallet* zBYTWallet)
{
    bool iszBYTWalletInitialized = (NULL != zBYTWallet);
    CMintMeta meta;
    ...
    if (! iszBYTWalletInitialized)
        zBYTWallet = new CzBYTWallet(strWalletFile);
    meta.isSeedCorrect = zBYTWallet->CheckSeed(dMint);
    if (! iszBYTWalletInitialized)
        delete zBYTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBYTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBYTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBYTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBYTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBYTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBYTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBYTWallet* zBYTWallet = new CzBYTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBYTWallet);
        }
        delete zBYTWallet;
        LogPrint("zero", "%s: added %d dzbyt from DB\n", __func__, listDeterministicDB.size());
    }
