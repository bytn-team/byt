...
CzBYTWallet::CzBYTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbyt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBYTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBYTSeed_deprecated()) {
                    LogPrintf("%s: Updated zBYT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbyt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbyt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBYTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBYTCount(nCountLastUsed);
    else if (!walletdb.ReadZBYTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBYTWallet::Lock()
{
    seedMaster = 0;
}

void CzBYTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBYTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBYT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBYTWallet::LoadMintPoolFromDB()
{
...
void CzBYTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBYTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBYTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbytTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBYTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBYT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbytTracker which also adds to database
    pwalletMain->zbytTracker->Add(dMint, true);
...
void CzBYTWallet::SeedToZBYT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBYTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBYTWallet::GenerateDeterministicZBYT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBYTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBYTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBYTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
