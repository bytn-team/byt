...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBYTModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbytTracker->HasMintTx(hash))) {
            //zBYT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBYT;
...
bool IsZBYTType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBYT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBYT || type == TransactionRecord::MNReward) {
