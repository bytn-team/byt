...
#include "ui_zbytcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbytTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbytTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBYT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBYT seed used to mint this zBYT is not the same as currently hold in the wallet";
            else
