...
public:
    ...
    bool WriteZBYTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBYTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBYTSeed_deprecated(uint256& seed);
    bool EraseZBYTSeed();
    bool EraseZBYTSeed_deprecated();

    bool WriteZBYTCount(const uint32_t& nCount);
    bool ReadZBYTCount(uint32_t& nCount);
