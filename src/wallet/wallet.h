...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbytspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BYT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBYT send
enum ZerocoinSpendStatus {
    ZBYT_SPEND_OKAY = 0,                            // No error
    ZBYT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBYT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBYT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBYT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBYT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBYT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBYT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBYT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBYT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBYT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBYT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBYT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBYT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBYT_SPENT_USED_ZBYT = 14,                      // Coin has already been spend
    ZBYT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBYT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBYTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbytAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBYTTracker> zbytTracker;
    ...
    void setZWallet(CzBYTWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbytTracker = std::unique_ptr<CzBYTTracker>(new CzBYTTracker(strWalletFile));
    }

    CzBYTWallet* getZWallet() { return zwalletMain; }
    ...
    /** zBYT reset */
    boost::signals2::signal<void()> NotifyzBYTReset;
